...
class TT_OrganizeMediaFolder_List_Table extends WP_List_Table {
	...
	private function read_data() {
		...
		if ( is_null( $search_text ) && get_user_option( 'omf_search_text', get_current_user_id() ) ) {
	...
	public function column_cb( $item ) {
		...
		return sprintf(
			'<input type="checkbox"' . $disabled . 'name="bulk_folder_check[]" value="%1$s" form="organizemediafolder_forms" />',
			/*%1$s*/ $item['ID']
		);
	}
